...
class ImageEmbeddingTestCase(unittest.TestCase):
    ...
    def test_image_is_embedded_even_with_mismatched_alt_text(self, mock_smtp_class):
        ...
        async def run_test():
            ...
            attachment = {"name": "test-image.png", "data": TEST_IMAGE_B64}
            email_data = (
            ...
            self.assertIsNotNone(
                image_part, "The image part was not found in the email."
            )
            ...
            self.assertIn(
                expected_img_tag_src,
                final_html,
                "The <img> tag's src was not replaced with the correct CID.",
            )
            self.assertNotIn(
                'src="placeholder.jpg"',
                final_html,
                "The original placeholder src should have been removed.",
            )
            self.assertIn(
                'alt="A nice picture"',
                final_html,
                "The original alt text was not preserved.",
            )
